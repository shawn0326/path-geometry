...
import type { BeveledCurveOptions, BuildFramesOptions3, Path3, PathFrames3, PointPreprocessOptions, PolylineOptions, Segment3, SmoothCurveOptions } from '../types';
...
import { segment3PointAt } from '../segments/shared';
import { frame3 } from '../frames/frame3';
import { clamp, EPSILON } from '../utils/math';
import { rotateAroundAxis } from '../utils/rotate';
import { getPathLength3, getPathLengths3, markPathDirty, path3PointAtDistance, path3TangentAtDistance } from './path-common';
...
export const path3 = {
   ...
   * @returns The same path object.
   */
  addSegment(path: Path3, segment: Segment3): Path3 {
   ...
   */
  markDirty(path: Path3, recursive = false): void {
   ...
   * @returns The same path object.
   */
  setPolylines(path: Path3, points: ReadonlyVec3[], options: PolylineOptions = {}): Path3 {
    path.segments.length = 0;
    ...
    for (let i = 0; i < segments; i++) {
      path.segments.push(line3.create(points[i]!, i === lastIndex ? points[0]! : points[i + 1]!));
    }
    markPathDirty(path);
    return path;
  },
   ...
   * @returns The same path object.
   */
  setSmoothCurves(path: Path3, points: ReadonlyVec3[], options: SmoothCurveOptions = {}): Path3 {
    ...
    for (let i = 0, l = points.length; i < l; i++) {
      ...
      if (i === 0) {
        ...
        path.segments.push(cubicBezier3.create(points[i - 1]!, cp0, current, current));
      } else {
        ...
        path.segments.push(cubicBezier3.create(prev, cp0, cp1, current));
   ...
   * @returns The same path object.
   */
  setBeveledCurves(path: Path3, points: ReadonlyVec3[], options: BeveledCurveOptions = {}): Path3 {
    ...
    for (let i = 0; i < segments; i++) {
      ...
      if (i === segments - 1 && !close) {
        path.segments.push(line3.create(p0, p1));
        vec3.copy(p0, p1);
        break;
      }
      ...
      path.segments.push(line3.create(p0, lineEnd));

      const bezierEnd = vec3.scaleAndAdd(vec3.create(), p1, nextDir, v2Dist);
      path.segments.push(quadraticBezier3.create(lineEnd, p1, bezierEnd));
   ...
   */
  getLength(path: Path3): number {
    return getPathLength3(path);
  },
   ...
   */
  getLengths(path: Path3): number[] {
    return getPathLengths3(path);
  },
   ...
   */
  pointAtU(out: vec3, path: Path3, u: number): vec3 {
   ...
   * @returns The out vector.
   */
  tangentAtU(out: vec3, path: Path3, u: number): vec3 {
   ...
   * @returns The out vector.
   */
  pointAtDistance(out: vec3, path: Path3, distance: number): vec3 {
    return path3PointAtDistance(out, path, distance);
  },
   ...
   * @returns The out vector.
   */
  tangentAtDistance(out: vec3, path: Path3, distance: number): vec3 {
    return path3TangentAtDistance(out, path, distance);
  },
   ...
   */
  getPoints(path: Path3, divisions = 12): vec3[] {
    const points: vec3[] = [];
    ...
    for (let i = 0; i < path.segments.length; i++) {
      ...
      for (let j = 0; j <= limit; j++) {
        ...
        segment3PointAt(point, segment, j / resolution);
        points.push(point);
      }
    }
    return points;
   ...
   */
  getSpacedPoints(path: Path3, divisions = 5): vec3[] {
    const points: vec3[] = [];
   ...
   */
  buildFrames(path: Path3, options: BuildFramesOptions3 = {}): PathFrames3 {
    ...
    frame3.initialNormal(normals[0], tangents[0], initialNormal);
    frame3.orthonormalize(normals[0], binormals[0], tangents[0], normals[0]);
    ...
    for (let i = 1; i < points.length - 1; i++) {
      ...
      if (frenet) {
        ...
        frame3.orthonormalize(normal, binormal, tangent, normal);
      } else {
    ...
    if (lastIndex > 0) {
      ...
      frame3.transportNormal(normal, normals[lastIndex - 1]!, tangents[lastIndex - 1]!, tangent);
      frame3.orthonormalize(normal, binormal, tangent, normal);
